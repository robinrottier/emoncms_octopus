...
for meter in  ["electricity", "gas"]:
	...
	settings_section = "octopus"
	emoncms_feed = "octopus"
	emoncms_name = meter
	emoncms_unit = "kWh"
	...
	result = requests.get(settings['emoncms']['server']+"/feed/getid.json",params={'tag':emoncms_feed,'name':emoncms_name,'apikey':settings['emoncms']['apikey']})
	if  not result.text:
	    # Create feed
	    params = {'tag':emoncms_feed,'name':emoncms_name,'datatype':1,'engine':5,'options':'{"interval":1800}','unit':emoncms_unit,'apikey':settings['emoncms']['apikey']}
	...
	url = "https://api.octopus.energy/v1/%s-meter-points/%s/meters/%s/consumption/" % (meter,settings[settings_section]['mpan'],settings[settings_section]['serial_number'])
	result = requests.get(url,params=params,auth=(settings[settings_section]['agile_apikey'],''))
